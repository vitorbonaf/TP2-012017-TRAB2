...
void ControllerAdmin::manageStudents() {
    /// Esta funcao exibe um menu para que o usuario selecione a acao desejada. Ap√≥s
    /// a selecao, √© chamada a funcao adequada para realizacao da acao desejada.
    int sel = -1;
...
void ControllerAdmin::removeStudent() {
	...
	while(sel < 0 || sel >= i){
		std::cout << "\n Usu·rio inv·lido. Informe novamento o n˙mero do usu·rio a ser removido: ";
		std::cin >> sel;
	}
...
void ControllerAdmin::includeSubject() {
    ...
    do{
		...
		if(!nameAvailable){
			std::cout << "\nNome indisponÌvel, disciplina j· existente. Escolha outro nome.\n\n";
		}
    }while(!nameAvailable);
            ...
            while(sel2 != 0) {
                ...
                if(sel2 != 0 && sel2 < i) {
                    ...
                    for(int k=0; k<10; k++) {
                        ...
                        std::cout << "A pergunta È de V ou F (1) ou M˙ltipla Escolha (2): ";
                        std::cin >> choice;
                        getline(std::cin, aux);
                        while(choice != 1 && choice != 2){
                        	std::cout << "OpÁ„o n„o existente, tente novamente. A pergunta È de V ou F (1) ou M˙ltipla Escolha (2): ";
                        	std::cin >> choice;
                        	getline(std::cin, aux);
                        }
                        ...
                        // solicitar value
                        while(ans != 'S' && ans != 'N'){
                        	std::cout << "VocÍ gostaria de atribuir um valor ‡ quest„o (S/N)? ";
                        	std::cin >> ans;
                        	getline(std::cin, aux);
                        }
                        ...
                        if (ans == 'S'){
                        	while(choice < 1 || choice > 10){
                        		std::cout << "DÍ um valor de 1-10 a quest„o: ";
								std::cin >> choice;
								getline(std::cin, aux);
                        	}
...
void ControllerAdmin::removeTopic() {
	...
	while(sel1 != 0) {
		system(CLEAR);
		printf("QuizTime - Excluir TÛpico\n\n");
		i = 1;
		std::vector<Subject*> *subjects = SubjectManager::instance()->getSubjects();
		//for(auto it:*(SubjectManager::instance()->getSubjects())){
		for(unsigned int j = 0; j < subjects->size(); j++){
		...
		std::cout << "0. Voltar\n";
		std::cout << "\nSelecione uma disciplina para excluir o tÛpico: ";
		std::cin  >> sel1;

		if(sel1 != 0 && sel1 < i) {
			while(sel2 != 0) {
				system(CLEAR);
				std::cout << "QuizTime - Excluir TÛpico\n\n";
				i = 1;
				for(unsigned int k = 0; k < subs_map[sel1]->getTopics()->size(); k++){
					Topic * top = subs_map[sel1]->getTopics()->at(k);
					std::cout << i <<". "<< top->getName() << "\n";
				...
				if(sel2 != 0 && sel2 < i) {
					...
					if(sel2 != 0) {
						std::cout << "Selecao de tÛpico invalida.\n";
						getchar();
					}
				}
			}
